...
hchart(p1_data, "bar", x = name_f, y = win_freq,color=group) %>% 
    hc_xAxis(title=list(text="Heros")) %>%
    hc_yAxis(title=list(text="Winning rate")) %>%
    hc_title(text = "Plot 1. Max/min winning rate with allies/enemies") %>%
...
hchart(inv_win_class, "bar", x = inv_class, y = freq) %>% 
    hc_xAxis(title=list(text="Build")) %>%
    hc_yAxis(title=list(text="Winning rate")) %>%
    hc_title(text = "Plot 2. Winning rate for different Inoker builds") %>%
    hc_add_theme(hc_theme_google()) %>%
    hc_chart(width=600) %>%
    hc_tooltip(valueDecimals=4)
